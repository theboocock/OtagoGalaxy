...
class DefaultToolAction( object ):
    ...
    def execute(self, tool, trans, incoming={}, return_job=False, set_output_hid=True, set_output_history=True, history=None, job_params=None, parralelism=None ):
        """
        ...
        job.tool_id = tool.id
        if trans.app.config.enable_clustering_interface:
            trans.app.job_manager.job_handler.dispatcher.clustering_interface.get_ui_reader().put(job.id,parralelism)
        try:
